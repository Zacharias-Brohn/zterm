...
use zterm::pty::Pty;
use zterm::renderer::{
    EdgeGlow, PaneRenderInfo, Renderer, StatuslineComponent, StatuslineContent,
    StatuslineSection,
};
use zterm::terminal::{
...
use winit::event::{
    ElementState, KeyEvent, Modifiers as WinitModifiers, MouseButton,
...
impl Pane {
    ...
    fn new(
        ...
        let pty =
            Pty::spawn(None, cols as u16, rows as u16, width_px, height_px)
    ...
    fn resize(
        ...
        let size_changed =
            cols != self.terminal.cols || rows != self.terminal.rows;
        ...
        if size_changed {
            if let Err(e) =
                self.pty
                    .resize(cols as u16, rows as u16, width_px, height_px)
            {
...
impl SplitNode {
        ...
        match self {
            ...
            SplitNode::Split {
                ...
                if *horizontal {
                    ...
                    let first_cols =
                        ((total_cols as f32) * *ratio).round() as usize;
                    let second_cols = total_cols.saturating_sub(first_cols);

                    // Convert back to pixel widths
                    let first_alloc_width =
                        first_cols.max(1) as f32 * cell_width;
                    let second_alloc_width =
                        second_cols.max(1) as f32 * cell_width;

                    // Layout panes flush against each other (border overlays the edge)
                    let (first_actual_w, first_actual_h) = first.layout(
                        x,
                        y,
                        first_alloc_width,
                        height,
                    ...
                    let (second_actual_w, second_actual_h) = second.layout(
                        x + first_actual_w,
                        y,
                        second_alloc_width,
                        height,
                    ...
                    (
                        first_actual_w + second_actual_w,
                        first_actual_h.max(second_actual_h),
                    )
                } else {
                    ...
                    let first_rows =
                        ((total_rows as f32) * *ratio).round() as usize;
                    let second_rows = total_rows.saturating_sub(first_rows);

                    // Convert back to pixel heights
                    let first_alloc_height =
                        first_rows.max(1) as f32 * cell_height;
                    let second_alloc_height =
                        second_rows.max(1) as f32 * cell_height;
    ...
    fn find_geometry(&self, target_id: PaneId) -> Option<PaneGeometry> {
        match self {
            ...
            SplitNode::Split { first, second, .. } => first
                .find_geometry(target_id)
                .or_else(|| second.find_geometry(target_id)),
        }
    }
        ...
        for (pane_id, geom) in all_geoms {
            ...
            if is_neighbor {
                ...
                let distance = match direction {
                    ...
                    Direction::Down => {
                        geom.y - (target_geom.y + target_geom.height)
                    }
                    Direction::Left => target_geom.x - (geom.x + geom.width),
                    Direction::Right => {
                        geom.x - (target_geom.x + target_geom.width)
                    }
                };
    ...
    fn contains_pane(&self, target_id: PaneId) -> bool {
        match self {
            ...
            SplitNode::Split { first, second, .. } => {
                first.contains_pane(target_id)
                    || second.contains_pane(target_id)
            }
        }
    }
...
impl Tab {
    ...
    fn resize(
        ...
        let used_dims = self.split_root.layout(
            0.0,
            0.0,
            width,
            height,
    ...
    fn split(
        ...
        // Update split tree
        let old_root =
            std::mem::replace(&mut self.split_root, SplitNode::leaf(PaneId(0)));
        self.split_root =
            old_root.split_pane(self.active_pane, new_pane_id, horizontal);
    ...
    fn focus_neighbor(&mut self, direction: Direction) {
        if let Some(neighbor_id) =
            self.split_root.find_neighbor(self.active_pane, direction)
        {
...
fn build_cwd_section(cwd: &str) -> StatuslineSection {
    ...
    for (i, segment) in segments.iter().enumerate() {
        ...
        if i > 0 {
            ...
            components
                .push(StatuslineComponent::new(" \u{E0B1} ").fg(prev_color));
        }
...
fn build_git_section(cwd: &str) -> Option<StatuslineSection> {
    ...
    components.push(
        StatuslineComponent::new(head_text)
            .rgb_fg(fg_color.0, fg_color.1, fg_color.2),
    );
    ...
    if status.ahead > 0 || status.behind > 0 {
        ...
        components.push(
            StatuslineComponent::new(branch_status)
                .rgb_fg(fg_color.0, fg_color.1, fg_color.2),
        );
    }
    ...
    if status.working_changed > 0 {
        ...
        components.push(
            StatuslineComponent::new(working_text)
                .rgb_fg(fg_color.0, fg_color.1, fg_color.2),
        );
    }
    ...
    if status.staging_changed > 0 {
        ...
        components.push(
            StatuslineComponent::new(staging_text)
                .rgb_fg(fg_color.0, fg_color.1, fg_color.2),
        );
    }
    ...
    if status.stash_count > 0 {
        ...
        components.push(
            StatuslineComponent::new(stash_text)
                .rgb_fg(fg_color.0, fg_color.1, fg_color.2),
        );
    }
...
impl Selection {
    ...
    fn to_screen_coords(
        ...
        let screen_end_row =
            (screen_end_row as usize).min(visible_rows.saturating_sub(1));
        let start_col = if start.row + scroll_offset < 0 {
            0
        } else {
            start.col
        };
        let end_col = if end.row + scroll_offset >= visible_rows as isize {
            usize::MAX
        } else {
            end.col
        };
...
impl App {
    ...
    fn reload_config(&mut self) {
        ...
        let font_size_changed =
            (new_config.font_size - self.config.font_size).abs() > 0.01;
        let opacity_changed = (new_config.background_opacity
    ...
    fn start_pane_io_thread(&self, pane: &Pane) {
        self.start_pane_io_thread_with_info(
            pane.id,
            pane.pty_fd,
            pane.shared_parser.clone(),
        );
    }
    ...
    fn start_pane_io_thread_with_info(
        &self,
        pane_id: PaneId,
        pty_fd: i32,
        shared_parser: Arc<SharedParser>,
    ...
    fn create_window(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let renderer =
            pollster::block_on(Renderer::new(window.clone(), &self.config));
    ...
    fn poll_pane(&mut self, pane_id: PaneId) -> (bool, bool) {
        ...
        for tab in &mut self.tabs {
            if let Some(pane) = tab.get_pane_mut(pane_id) {
                ...
                ever_processed =
                    pane.shared_parser.run_parse_pass(&mut pane.terminal);
    ...
    fn handle_terminal_command(
        ...
        match cmd {
            TerminalCommand::NavigatePane(direction) => {
                log::debug!(
                    "Terminal requested pane navigation: {:?}",
                    direction
                );
            ...
            TerminalCommand::SetClipboard(content) => {
                log::debug!(
                    "OSC 52: Setting clipboard ({} bytes)",
                    content.len()
                );
                // Use wl-copy to set the clipboard on Wayland
                    ...
                    .spawn()
                {
                    Ok(mut child) => {
                        if let Some(stdin) = child.stdin.as_mut() {
                            ...
                            {
                                log::warn!("Failed to write to wl-copy: {}", e);
                            }
                        }
                        // Don't wait for child to exit - let it run async
                    }
                    Err(e) => {
                        log::warn!("Failed to spawn wl-copy for OSC 52: {}", e);
                    }
                }
            }
        }
    ...
    fn do_render(&mut self) -> bool {
        ...
        if let Some(tab) = self.active_tab_mut() {
            if let Some(pane) = tab.active_pane_mut() {
                ...
                if scrollback_len != pane.last_scrollback_len {
                    let lines_added = scrollback_len
        ...
        if let Some(renderer) = &mut self.renderer {
            if let Some(tab) = self.tabs.get_mut(active_tab_idx) {
                ...
                if let Some(pane) = tab.panes.get_mut(&active_pane_id) {
                    if pane.custom_statusline.is_some() {
                        if let Some(proc_name) =
                            pane.pty.foreground_process_name()
                        {
                            let is_vim = proc_name == "nvim"
                                || proc_name == "vim"
                                || proc_name == "vi";
                            if !is_vim {
                ...
                let animation_in_progress =
                    dim_factors.iter().any(|(id, factor)| {
                ...
                let image_animation_in_progress = tab
                    .panes
                    .values()
                    .any(|pane| pane.terminal.image_storage.has_animations());

                needs_another_frame = animation_in_progress
                    || glow_in_progress
                    || image_animation_in_progress;

                // Get the statusline content for the active pane
                let statusline_content: StatuslineContent = tab
                    .panes
                    .get(&active_pane_id)
                    .map(|pane| {
    ...
    fn focus_pane(&mut self, direction: Direction) {
        ...
        if !navigated {
            ...
            if let (Some(geom), Some(renderer)) =
                (active_pane_geom, &self.renderer)
            {
                let (glow_x, glow_y, glow_width, glow_height) = renderer
                    .calculate_edge_glow_bounds(
                        geom.x,
                        geom.y,
                        geom.width,
                        geom.height,
    ...
    fn paste_from_clipboard(&mut self) {
        ...
        if output.status.success() && !output.stdout.is_empty() {
            let bracketed_paste = self
                ...
                .map(|p| p.terminal.bracketed_paste)
                .unwrap_or(false);
    ...
    fn copy_selection_to_clipboard(&mut self) {
        ...
        let screen_end_row = ((end.row + scroll_offset).max(0) as usize)
        ...
        for screen_row in screen_start_row..=screen_end_row {
            ...
            let col_start = if content_row == start.row {
                start.col
            } else {
                0
            };
            let col_end = if content_row == end.row {
                end.col
            } else {
    ...
    fn handle_keyboard_input(&mut self, event: KeyEvent) {
        ...
        if event_type == KeyEventType::Release
            && !self.keyboard_state.report_events()
        {
        ...
        // Get the application cursor keys mode from the active terminal
        let application_cursor_keys = self
            .active_tab()
            .and_then(|t| t.active_pane())
            .map(|p| p.terminal.application_cursor_keys)
            .unwrap_or(false);

        let encoder = KeyEncoder::with_cursor_mode(
            &self.keyboard_state,
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: UserEvent) {
        match event {
            ...
            UserEvent::Tick => {
                ...
                let should_render = any_input
                    && any_not_synchronized
                ...
                #[cfg(feature = "render_timing")]
                let render_time = if should_render {
        ...
        match event {
            ...
            WindowEvent::Focused(focused) => {
                let focus_reporting = self
                    .active_tab()
                    .and_then(|t| t.active_pane())
                    .map(|p| p.terminal.focus_reporting)
                    .unwrap_or(false);
...
fn setup_config_watcher(
    ...
    let config_path = match Config::config_path() {
        ...
        None => {
            log::warn!(
                "Could not determine config path, config hot-reload disabled"
            );
            return None;
        }
    };
                    ...
                    match event.kind {
                        EventKind::Modify(_) | EventKind::Create(_) => {
                            ...
                            if is_config_file {
                                ...
                                let _ =
                                    proxy.send_event(UserEvent::ConfigReloaded);
                            }
                        }
                        _ => {}
                    }
